# from: https://github.com/ryayoung/toolcall/tree/main
    ...
    def decorator(fn):
        ...
        for name in blacklisted_param_names:
            if name in param_names:
                raise ValueError(f"Parameter name '{name}' is reserved for OpenAI functions.")

        param_defaults = {
            param.name: param.default if param.default is not param.empty else ... for param in inspected_params
        }
        ...
        class Model(Base, metaclass=BaseMeta):
            ...
            @classmethod
            def run_tool_call(cls, tool_call, error_handler: bool | Callable[[Exception], str] = False):
        ...
        positional_only_params = [p.name for p in inspected_params if p.kind == inspect.Parameter.POSITIONAL_ONLY]
...
def get_schema(
    ...
    if doc.params:
        descriptions = {
            ...
            if param.description and param.arg_name and param.arg_name in parameters["properties"]
        }
    ...
    for key, prop in parameters["properties"].items():
        parameters["properties"][key] = order_properties(prop, start=["type", "anyOf", "description"], end=["default"])
...
def remove_null_annotations(prop: dict) -> None:
    ...
    if "anyOf" in prop:
        prop["anyOf"] = [tp for tp in prop["anyOf"] if not (isinstance(tp, dict) and tp.get("type") == "null")]
        if len(prop["anyOf"]) == 1:
